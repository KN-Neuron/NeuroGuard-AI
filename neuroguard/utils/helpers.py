...
import os
from neuroguard.commons.constant import RESULTS_FOLDER
...
def create_writer(
    ...
    Example usage:
        # Create a writer saving to "neuroguard/data/result_summaries/2025-03-15/data_10_percent/EEGNet/50_epochs/"
        writer = create_writer(experiment_name="data_10_percent",
                               model_name="EEGNet",
                               extra="50_epochs")
        # The above is the same as:
        writer = SummaryWriter(log_dir="neuroguard/data/result_summaries/runs/2025-03-15/data_10_percent/EEGNet/50_epochs/")
    """
...
def predict_ids(embeddings_2d: np.ndarray[Any, Any], user_profiles: Dict[str, np.ndarray[Any, Any]]) -> List[str]:
    ...
    for embedding in embeddings_2d:
        ...
        closest_participant = min(
            user_profiles.keys(),
            key=lambda pid: np.linalg.norm(embedding - user_profiles[pid]),
        )
