            ...
            for notch_filter in notch_filters:
                for tmin in tmins:
                    for tmax in tmaxs:
                        ...
                        extractor = EEGDataExtractor(data_dir = 'data/Kolory', lfreq = lfreq, hfreq = hfreq, notch_filter = notch_filter,
                                                     tmin = tmin, tmax=tmax)

                        df, participants = extractor.extract_dataframe()
                        df_labels = df['participant_id']
                        X, y, le = encode_features_and_labels(df)
                        X_train, y_train, X_test, y_test, train_labels, test_labels = split_by_user(X, y, df_labels, random_state=seed)
                        ...
                        for emb_dim in emb_dims:
                            ...
                            for margin in margins:
                                ...
                                for epochs in n_epochs_list:
                                    current_iteration+=1
                                    final_loss = train_triplet_online(model, train_loader, criterion, optimizer, device, n_epochs=epochs)
                                    embeddings, labels = extract_embeddings_online(model, test_loader, device)

                                    print(f"{current_iteration}/{combinations}, Loss: {final_loss}")
                                    result = {
                                        "params": {
                                            "lfreq": lfreq,
                                            "hfreq": hfreq,
