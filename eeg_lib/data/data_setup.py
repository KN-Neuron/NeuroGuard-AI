...
class TripletEEGDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        anchor_tensor = torch.tensor(anchor_row['epoch'], dtype=torch.float32).unsqueeze(0)
        
        pos_index = index
        while pos_index == index:
            pos_index = random.choice(self.participants_dict[anchor_pid])
        pos_row = self.df.iloc[pos_index]
        positive_tensor = torch.tensor(pos_row['epoch'], dtype=torch.float32).unsqueeze(0)
        ...
        negative_tensor = torch.tensor(neg_row['epoch'], dtype=torch.float32).unsqueeze(0)
