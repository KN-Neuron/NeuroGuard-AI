...
model = BasicModel(input_size=train_dataset.input_size, num_classes=train_dataset.num_classes, loss_relation=.7)

model.load_state_dict(torch.load("coh_model1.pth", weights_only=True, map_location=torch.device("cuda")))
model.to(torch.device("cuda"))

model.train_model(train_loader, val_loader=val_loader, lr=.001, epochs=50)
