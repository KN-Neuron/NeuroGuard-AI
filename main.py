...
class CustomDatasetP300(Dataset):
    def __init__(self):
        super().__init__()
...
class CustomDatasetKolory(Dataset):
    def __init__(self):
        ...
        self.X = torch.tensor([*eeg_df["epoch"].to_numpy()], dtype=torch.float32)
...
for epoch in range(30):
    ...
    print(
        f"Epoch [{epoch+1:2}/50], Loss: {epoch_loss:.4f}, Accuracy: {correct / total:.2f}"
    )
